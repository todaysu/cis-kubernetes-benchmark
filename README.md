...
项目包含完整的测试框架，使用 BATS (Bash Automated Testing System) 进行自动化测试。
...
| 测试类型 | 说明 | 覆盖范围 |
|---------|------|----------|
| **单元测试** | 测试独立函数 | 文件检查、参数解析、输出过滤 |
| **集成测试** | 测试组件交互 | 三层检查框架、完整检查流程 |
| **E2E测试** | 端到端测试 | 在 Kind 集群中运行真实审计 |
...
测试结果：[GitHub Actions](https://github.com/todaysu/cis-kubernetes-benchmark/actions)

### Mock 数据

项目包含完整的 Mock 数据，用于测试而无需真实 Kubernetes 集群：
...
Mock 数据包括：
- 模拟进程输出（API Server、Kubelet、etcd 等）
- 模拟 kubectl 输出
- 模拟配置文件
- 测试场景（合规/不合规/部分合规）
...
查看 [tests/README.md](tests/README.md) 了解如何编写新测试。
